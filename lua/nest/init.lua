...
function module.applyKeymaps(config, presets)
	local mergedPresets = mergeOptions(presets or module.defaults, config)
	...
	if type(first) == 'table' then
		...
		return vim.tbl_isempty(appliedConfig) and nil or appliedConfig
	end
...
function module.revertKeymaps(config, presets)
	local mergedPresets = mergeOptions(presets or { prefix = '' }, config)
